  ...
  init({ imageSizes, optimizeOptions, settings, ...config }) {
    ...
    function getDeleteAWS(file) {
      return new Promise(async (resolve, reject) => {
        ...
        file.path = file.formats?.thumbnail.path;
    ...
    function uploadLocalStream(file, customParams = {}) {
      return new Promise((resolve, reject) => {
        ...
        if(checkJson(file.path)){
          let customsUserValues = JSON.parse(file.path);
          file.path = customsUserValues.upload_path;
          file.userSizes = customsUserValues.imageSizes ? customsUserValues.imageSizes.split(',') : undefined;
        }
    ...
    function uploadLocal(file, customParams = {}) {
      return new Promise((resolve, reject) => {
        ...
        if(checkJson(file.path)){
          let customsUserValues = JSON.parse(file.path);
          file.path = customsUserValues.upload_path;
          file.userSizes = customsUserValues.imageSizes.split(',') || [];
        }
    ...
    function uploadLocalSizes(localOriginal){
      return new Promise((resolve, reject) => {
        ...
        if(fileFormat === 'image' && fileType !== 'thumbnail') {
          // Original Image
