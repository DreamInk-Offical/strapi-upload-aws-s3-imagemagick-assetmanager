  ...
  init({ imageSizes, optimizeOptions, settings, ...config }) {
    ...
    function uploadLogic(file, customParams = {}) {
      return new Promise(async (resolve, reject) => {
        createTemp;
        // Upload Image
        let localOriginal = await uploadLocal(file, customParams = {});
        ...
        // Get file stream or??? buffer for each size
    ...
    function uploadLocal(file, customParams = {}) {
      return new Promise((resolve, reject) => {
        ...
        if(file.stream) {
          file.buffer = fs.readFileSync(file.stream.path);
        };
    ...
    function removeTemp(file) {
      return new Promise((resolve, reject) => {
        fs.unlinkSync(file.stream.path) 
        resolve();
      });
    };
