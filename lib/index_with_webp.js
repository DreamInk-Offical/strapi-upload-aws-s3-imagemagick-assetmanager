...
module.exports = {
  init({ imageSizes, optimizeOptions, settings, ...config }) {
    ...
    const S3BaseUrl = config.endpoint ? config.endpoint : `https://${config.params.Bucket}.s3.${config.region}.amazonaws.com`;
        ...
        return new Promise(async (resolve, reject) => {
          if(fileFormat === 'image') {
            ...
            for (let item of buffers) {
              await S3.upload(
                {
                  Key: file.path ? `assets/${file.path}/${item.path}` : `assets/${item.path}`,
        ...
        return new Promise(async (resolve, reject) => {
          if(fileFormat === 'image') {
            await S3.deleteObject(
              { Key: file.path ? `assets/${file.path}/${fileType}/${file.hash}${file.ext}` : `assets/${fileType}/${file.hash}${file.ext}`, ...customParams, },
              (err, data) => {
            ...
            if (file.hash.split('_')[0] !== 'thumbnail') {
              for (let size of imageSizes) {
                if (file.ext !== '.webp') {
                  await S3.deleteObject(
                    { Key: file.path ? `assets/${file.path}/${size.name}/${file.hash}${file.ext}` : `assets/${size.name}/${file.hash}${file.ext}`, ...customParams, },
                    (err, data) => {
                      if (err) { console.error(err); return; }
                ...
                if (size.isGenerateWebp) {
                  await S3.deleteObject(
                    { Key: file.path ? `assets/${file.path}/${size.name}/${file.hash}.webp` : `assets/${size.name}/${file.hash}.webp`, ...customParams, },
                    (err, data) => {
                      if (err) { console.error(err); return; }
